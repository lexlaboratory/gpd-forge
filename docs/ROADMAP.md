...
| ~~E2E suite fails intermittently, cause unknown~~ | fixed 2026-09-25 | **Loopback connection churn tripping a host-level stall, not the mock.** The mock's new request log (`MOCK_LOG`, `tools/mock-daemon/diagnostics.mjs`) showed every request answered in ≤ 1 ms, no event-loop stall, no hung request — and the failing `request.get` never reaching it. Reproduced with no Playwright and no mock: a bare `net.connect` loop against a trivial listener gets windows of 10–35 s in which **every** new connect to 127.0.0.1 — to two different listeners and to other apps' — fails with `ETIMEDOUT` after ~310 ms, while established sockets keep working. Onset tracks connection count: never at 5 connects/s for 5 min, after ~130 s at bursts of 10 every 700 ms (the suite's own pattern), within seconds at 250/s. The suite made ~600 sockets to the mock and ~400 to the preview server per run (a fresh browser context per test), right at that threshold. Which kernel component drops the SYN (Defender network inspection and Tailscale's WFP filters are both present) needs an elevated trace and was not identified. **Fix:** `reuseContext: true` (one context per worker, reset between tests) → ~18 sockets a run; 3 consecutive 184/184 runs, 1.3 min instead of 2–3 (before: 4 of 5 runs failed). `connection-churn.spec.ts` guards the socket count; video recording is off because it disables reuse. **Correction (F1 audit round 3, 2026-09-25): the suite does not trigger the stall.** Failures came back at ~45 mock sockets a run. A probe opening one connect a second to its own listener, beside four full runs: 22 failed connects (`ETIMEDOUT` after ~305 ms) in windows of 2–11 s, the first 10 s into a run and the last 11–20 s after the suite had exited; netstat sampled each second counted ~1,000 new loopback connections machine-wide in those ~7 min (Orca, dev servers, the installed daemon) of which the suite's were ~30 to the mock and ~15 to the preview server, at ~2/s overall. So the stall is the host's and no socket budget prevents it; what the suite controls is how many new connects it needs, and what happens when one fails. **Fix:** the mock and `vite preview` keep idle sockets for 10 min (Node's 5 s default closed the request fixture's socket between tests, so each test's setup reconnected); `tests/e2e/fixtures.ts` retries a connect that failed before the request left (`connect E…` in the API fixture, `net::ERR_CONNECTION_*` on `page.goto`/`reload`) for up to 40 s; the test timeout is 60 s to leave room for it. |
| ~~Check BIOS setup for a charge threshold~~ | closed 2026-09-03 | **The option exists and does nothing** — recorded in [ADR-0004](adr/0004-no-charge-threshold-on-this-board.md) (commit `53e6151`). The question is settled; no EC dump-and-diff to build. |
