...
| ⚠️ Check BIOS setup for a charge threshold | *(open, needs Alex, ~10 min)* | Several GPD handhelds expose a charge limit in BIOS setup (hold `DEL` at boot). If this board has one, the right shape is what `GET /firmware` already does — report it and say where it is, write nothing — and it makes the EC research below unnecessary. Cheapest open item in the project. |
